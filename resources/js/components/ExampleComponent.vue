 ...
 <!-- <apexchart type=bar height=350 :options="chartOptions" :series="series" / -->
    ...
    >
      ...
      <v-container>
              <v-row>
                  <v-col cols='1'>
                     <v-img :src="getImagePath(item)" aspect-ratio="1" height=100 width=100 class="grey lighten-2"></v-img>
                  </v-col>
                  <v-col cols='11'>
            ...
            <template v-slot:default="{ active, toggle }">
              <v-list-item-content>
                ...
                <v-container>
                  <v-row>
                    <v-col cols='1'>
                         <v-img :src="getImagePath2(location)" aspect-ratio="1" height=50 width=50 class="grey lighten-2"></v-img>
                    </v-col>
                      <v-col cols='11'>
                         <v-list-item-title v-text="location.name"></v-list-item-title>
                         <v-list-item-subtitle v-text="location.description"></v-list-item-subtitle>
                    </v-col>
                  </v-row>
                </v-container>
...
<script>
  ...
  export default {
    ...
    methods:{
          ...
          borders.forEach((item,key)=>{
             let polygon = new google.maps.Polygon({
              ...
              polygon.addListener('click', (event)=>{
                this.dataDusun[key+1].infowindow.active=!this.dataDusun[key+1].infowindow.active;
                if(this.dataDusun[key+1].infowindow.active){
                  this.dataDusun[key+1].infowindow.data=new google.maps.InfoWindow;
                  this.dataDusun[key+1].infowindow.data.setContent("<b>Dusun "+this.dataDusun[key+1].name+"</b>");
                  this.dataDusun[key+1].infowindow.data.setPosition(event.latLng);
                  this.dataDusun[key+1].infowindow.data.open(this.current.Map);
                }else{
                   this.dataDusun[key+1].infowindow.data.close();
                }
               
                

              });
              this.dataDusun[key+1].polygon=polygon;
        ...
        getImagePath2:function(data){
          let folder;
          switch(data.jenis.name){
            case "Perangkat Desa":
              folder="perangkat_desa";
              break;
            case "UMKM":
              folder="umkm";
              break;
          ...
          return "img/"+folder+"/"+data.dusun.name+".png";
        },
        ...
        changeMap:function(data){
             this.dataDusun.forEach((item,key)=>{
              item.infowindow={active:false, data:null};
              item.polygon=null;
            });
            this.content = 'map';
