...
class BasedGendersTableSeeder extends Seeder
{
    ...
    public function run()
    {
       ...
       foreach ($dusuns as $key => $value) {
       		$index = $get->search(function($item,$key)use($value){
       			return $item->name==$value['name'];
       		});
       		//echo $get[$index]->name."\n";
       		foreach($value['data'] as $key2 => $value2){
       			$data = ['dusun_id'=>$get[$index]->id, 'rukun_tetangga_id'=>$value2['rt'],'gender'=>'L','jumlah'=>$value2['penduduk']['L']];
       			$data2 = ['dusun_id'=>$get[$index]->id, 'rukun_tetangga_id'=>$value2['rt'],'gender'=>'P','jumlah'=>$value2['penduduk']['P']];
